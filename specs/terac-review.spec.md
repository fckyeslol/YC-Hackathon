...
- **Estado:** 🟢 Implementado y cableado end-to-end‡ (2026-07-24). ✅ En el composition root (`server/index.ts`): escalada → `ReviewCoordinator.stageConsent` → preview → 👍/"sí" → **gate estructural `escalateToReviewers` (leakCheck + `ConsentToken.matches`, BR-A8)** → recruit Terac (best-effort) → página `GET/POST /review/:pseudonym` → juicios → `computeConsensus` (Dawid–Skene) → coaching al usuario. Módulos + tests (23 archivos, todo verde): `getSubmissionAnswers` (BR-T3), canal `"terac"`, `harvest.ts`, `pendingReview.ts`, `reviewPage.ts`, `reviewCoordinator.ts`, `teracDelivery.ts`.
  - ‡ **Recolección por la página propia** (flujo A): los revisores responden en `/review/:pseudonym` y el coordinator agrega. `harvest.ts` (pull de `getSubmissionAnswers` de Terac, flujo B) queda **testeado pero no cableado** como alternativa si Terac hospeda el formulario.
  - ⏳ **BR-T9 pendiente**: verificar en vivo `createOpportunity`/`getSubmissionAnswers` contra la API real (el recruit es best-effort con try/catch; un fallo no rompe el loop). Rúbrica QA opcional (los tests ya cubren §3).
